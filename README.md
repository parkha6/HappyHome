...
플레이어와 몬스터가 서로 충돌할지, 통과할지, 넉백이 발생할지 등은
Layer, Collision Matrix, OnTrigger… 함수 등을 조합해 처리합니다.

몬스터 Die() 시점에 드랍 생성 후 Destroy(gameObject)로 제거하며,
SpawnDrop() 메서드를 통해 골드·스킨 조각·윙 조각 등을 상황에 따라 드랍합니다.

상황에 따라 플레이어가 몬스터와 충돌했을 때
